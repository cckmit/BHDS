...
public interface EMAILREP extends JpaRepository<EmailAlert, String> {
	...
	@Query(value = "select count(*) from BHDS_EMAIL_NOTIFICATION_TABLE where send_flg ='N'", nativeQuery = true)
	int getEmailSentCount();
	
	
	@Query(value = "SELECT * FROM BHDS_EMAIL_NOTIFICATION_TABLE where send_flg='N'", nativeQuery = true)
	List<EmailAlert> getEmailDetails();
	
	@Query(value = "SELECT * FROM BHDS_EMAIL_NOTIFICATION_TABLE where email_srl_no = ?1", nativeQuery = true)
