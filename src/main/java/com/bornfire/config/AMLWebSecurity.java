	...
	public AuthenticationProvider authenticationProvider() {
		...
		DaoAuthenticationProvider ap = new DaoAuthenticationProvider() {
			...
			public Authentication authenticate(Authentication authentication) throws AuthenticationException {
				...
				UserProfileResponse up = userProfileRep.checkUser(userid);
			
				try {

					if (up.getStatus().equals("success")) {
						UserProfile usr = up.getUserProfile();
	...
	public UserDetailsService userDetailsService() {
		...
		return new UserDetailsService() {
			...
			public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
				...
				UserProfileResponse up = userProfileRep.checkUser(username);


				if (up.getStatus().equals("success")) {					
					System.out.println("outside---->"+up.getUserProfile().getEmpid());
					return up.getUserProfile();
	...
	public AuthenticationSuccessHandler amlAuthSuccessHandle() {
		return new AuthenticationSuccessHandler() {
			...
			public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
				...
				UserProfileResponse up = userProfileRep.checkUser(authentication.getName());
				UserProfile user = up.getUserProfile();
				user.setNo_of_attmp(0);
				user.setUser_locked_flg("N");
				userProfileRep.updateLoginAttempt(user);
